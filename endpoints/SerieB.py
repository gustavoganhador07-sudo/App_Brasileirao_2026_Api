...
SerieB_bp = Blueprint("SerieB", __name__, url_prefix="/SerieB")
...
def criar_SerieB():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO SerieB (NomeClube, PontosClube, JogosClube, SaldosGols,VitoriasClube,EmpateClube,DerrotasClube,PosicaoClube)"
        "VALUES (?, ?, ?, ?, ?, ?, ? ,?)",
...
##ROTA UPDATE
#############################################
@SerieB_bp.route("/<int:idSerieB>", methods=["PUT", "PATCH"])
...
@SerieB_bp.route("/<int:idSerieB>", methods=["DELETE"])
def deletar_SerieB(idSerieB):
    conn = conectar()
    cursor = conn.cursor()

    # tenta apagar o registro informado
    cursor.execute("DELETE FROM SerieB WHERE idSerieB = ?", (idSerieB,))
    conn.commit()
