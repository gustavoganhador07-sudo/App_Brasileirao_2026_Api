...
##ROTA UPDATE
#############################################
@app.route("/SerieA/<int:idSerieA>", methods=["PUT", "PATCH"])
...
@app.route("/SerieA/<int:idSerieA>", methods=["DELETE"])
...
##ROTA GET
##############################################
@app.route("/SerieB", methods=["GET"])
def listar_SerieB():
    conn = conectar()
    #conn.execute("PRAGMA foreign_keys = ON") #ativa as chaves estrangeiras das tabelas (pois, não é ativado por padrão)
    cursor = conn.cursor()
    cursor.execute("SELECT idSerieB, NomeClube, PontosClube, JogosClube,Saldosgols,VitoriasClube,EmpateClube,DerrotasClube,PosicaoClube FROM SerieB")
    dados = [
        {"idSerieB": row[0], "NomeClube": row[1], "PontosClube": row[2], "JogosClube": row[3], "Saldosgols": row[4], "VitoriasClube": row[5], "EmpateClube": row[6], "DerrotasClube": row[7], "PosicaoClube": row[8]}
        for row in cursor.fetchall()
    ]
    conn.close()
    return jsonify(dados)
...
def criar_serieB():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO SerieB (NomeClube, PontosClube,saldosGols,VitoriasClube,EmpateClube,DerrotasClube,PosiacaoClube)"
        "VALUES (?, ?, ?, ?, ?, ?, ? ,?)",
        (dados["NomeClube"], dados["PontosClube"],dados["JogosClube"], dados["SaldosGols"],dados["vitoriasClube"],dados["EmpateClube"],dados["derrotasClube"],dados["PosicaoClube"])
    )
    conn.commit()
    ...
    resposta.status_code = 201
    resposta.headers["Location"] = f"/SerieB/{novo_id}"
    return resposta

##ROTA UPDATE
#############################################
@app.route("/SerieB/<int:idSerieB>", methods=["PUT", "PATCH"])
def atualizar_SerieB(idSerieB):
    dados = request.get_json(silent=True)
    if not dados:
        abort(400, description="JSON inválido ou ausente")
    ...
    cursor = conn.cursor()
    cursor.execute(
        f"UPDATE SerieB SET {', '.join(set_clauses)} WHERE idSerieB = ?",
        tuple(valores)
    )
    conn.commit()
...
@app.route("/SerieB/<int:idSerieB>", methods=["DELETE"])
def deletar_SerieB(idSerieB):
    conn = conectar()
    cursor = conn.cursor()

    # tenta apagar o registro informado
    cursor.execute("DELETE FROM CadastroUsuario WHERE IDCadastroUsuario = ?", (idSerieB,))
    conn.commit()
...
def criar_SerieC():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO SerieC (NomeClube, PontosClube,JogosClube,SaldosGolsVitorasClube,DerrotasClube,EmpateClube,PosicaoClube)"
        "VALUES (?, ?, ? ,? ,? ,? ,? ,?)",
    ...
    resposta.status_code = 201
    resposta.headers["Location"] = f"/SerieC/{novo_id}"
    return resposta

##ROTA UPDATE
#############################################
@app.route("/SerieC/<int:idSerieC>", methods=["PUT", "PATCH"])
def atualizar_SerieC(idSerieC):
    dados = request.get_json(silent=True)
    if not dados:
        abort(400, description="JSON inválido ou ausente")
    ...
    cursor = conn.cursor()
    cursor.execute(
        f"UPDATE SerieC SET {', '.join(set_clauses)} WHERE idSerieC = ?",
        tuple(valores)
    )
    conn.commit()
...
@app.route("/SerieC/<int:idSerieC>", methods=["DELETE"])
def deletar_serieC(idSerieC):
    conn = conectar()
    cursor = conn.cursor()

    # tenta apagar o registro informado
    cursor.execute("DELETE FROM CadastroUsuario WHERE IDCadastroUsuario = ?", (idSerieC,))
    conn.commit()
...
##ROTA GET
##############################################
@app.route("/SerieD", methods=["GET"])
def listar_Cadastros():
    conn = conectar()
    #conn.execute("PRAGMA foreign_keys = ON") #ativa as chaves estrangeiras das tabelas (pois, não é ativado por padrão)
    cursor = conn.cursor()
    cursor.execute("SELECT idSerieD, NomeClube, PontosClube, JogosClube,Saldosgols,VitoriasClube,EmpateClube,DerrotasClube,PosicaoClube FROM SerieD")
    dados = [
        {"idSerieD": row[0], "NomeClube": row[1], "PontosClube": row[2], "JogosClube": row[3], "Saldosgols": row[4], "VitoriasClube": row[5], "EmpateClube": row[6], "DerrotasClube": row[7], "PosicaoClube": row[8]}
        for row in cursor.fetchall()
    ]
    conn.close()
    return jsonify(dados)
...
def criar_usuario():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO SerieD ({NomeClube, PontosClube,JogosClube,SaldosGols,VitoriasClube,EmpateClube,DerrotasClube,PosicaoClube,)"
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        (dados["NomeUsuario"], dados["PontosClube"], dados["JogosClube"],dados["SaldosGols"],dados["VitoriasClube"],dados["EmpateClube"],dados["DerrotasClube"],dados["PosicaoClube"])
    )
    conn.commit()
    novo_id = cursor.lastrowid
    conn.close()
...
##ROTA UPDATE
#############################################
@app.route("/SerieD/<int:idSerieD>", methods=["PUT", "PATCH"])
def atualizar_usuario(idSerieD):
    dados = request.get_json(silent=True)
    if not dados:
        abort(400, description="JSON inválido ou ausente")
...
@app.route("/SerieD/<int:idSerieD>", methods=["DELETE"])
def deletar_usuario(idSerieD):
    conn = conectar()
    cursor = conn.cursor()
